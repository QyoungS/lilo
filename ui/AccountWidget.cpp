...
void AccountWidget::onAdd() {
    ...
    connect(ok,  &QPushButton::clicked, dlg, [=]() {
        if (!ValidationHelper::isNotEmpty(nameEdit->text())) {
            QMessageBox::warning(dlg, "오류", "계좌명을 입력하세요.");
            return;
        }
        if (!ValidationHelper::isValidAccountNumber(numEdit->text())) {
            QMessageBox::warning(dlg, "오류", "계좌번호는 6~20자리 숫자/하이픈이어야 합니다.");
            return;
        }
        if (!AccountManager::instance().createAccount(m_userId, nameEdit->text(),
                numEdit->text(), typeBox->currentText(), curBox->currentText())) {
            QMessageBox::warning(dlg, "오류", "이미 존재하는 계좌번호입니다.");
            return;
        }
        dlg->accept();
    });
...
void AccountWidget::onEdit() {
    ...
    curBox->setCurrentText(a.currency);
    ...
    connect(ok,  &QPushButton::clicked, dlg, [=]() {
        ...
        AccountManager::instance().updateAccount(id, nameEdit->text(),
            typeBox->currentText(), curBox->currentText());
        dlg->accept();
    });
    dlg->exec();
    dlg->deleteLater();
...
void AccountWidget::onDelete() {
    ...
    if (QMessageBox::question(this, "삭제", "이 계좌와 모든 거래 내역을 삭제하시겠습니까?")
            == QMessageBox::Yes) {
        AccountManager::instance().deleteAccount(id);
    }
}

static void showDepositWithdrawDialog(QWidget* parent, int accountId, bool isDeposit) {
...
void AccountWidget::onDeposit() {
    int id = selectedAccountId();
    if (id < 0) { QMessageBox::information(this, "선택", "계좌를 선택하세요."); return; }
    showDepositWithdrawDialog(this, id, true);
}

void AccountWidget::onWithdraw() {
    int id = selectedAccountId();
    if (id < 0) { QMessageBox::information(this, "선택", "계좌를 선택하세요."); return; }
    showDepositWithdrawDialog(this, id, false);
}
